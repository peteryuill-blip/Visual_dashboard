...
## 2026-05-20 - Memoizing React components over large list mappings to avoid O(N) Array rendering issues
**Learning:** Found a major UI performance bottleneck rendering out a large amount of UI nodes whenever the parent array values updated, doing an O(N) layout shift.
**Action:** Isolate these lists mappings and componentize them into smaller stateless functions using `React.memo()`. Also ensure `useCallback` handles functions being sent downwards so its reference values are kept structurally stable through state updates.
