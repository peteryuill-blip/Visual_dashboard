...
export default function OutputReview({ outputText, image, onNext, onReRun, onDownload }) {
  ...
  // Memoize parsing logic to prevent unnecessary re-evaluations of expensive regex and string operations on tab switch
  // This avoids recalculating the JSON matching and replacements every time the component re-renders.
  const { jsonContent, websiteContent, proseContent } = useMemo(() => {
    ...
    const json = jsonMatch ? jsonMatch[1] : "No JSON found.";

    // Let's just do a rough split for website
    const website = outputText.includes('The Mark') ? outputText.substring(outputText.indexOf('The Mark')) : "ARCHIVE_ONLY - No website content generated";

    // Raw prose is everything without json
    const prose = outputText.replace(/```json\n[\s\S]*?\n```/, '');

    return { jsonContent: json, websiteContent: website, proseContent: prose };
  }, [outputText]);
