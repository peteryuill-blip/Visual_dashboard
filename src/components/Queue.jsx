...
// ⚡ Bolt: Extracted image row rendering into a React.memo component.
// This prevents O(n) re-renders of the large image list every time the Queue component updates,
// significantly improving rendering performance during large file batch selections.
const ImageItem = React.memo(({ img, onClick }) => {
  return (
    <div
  ...
  return (
    <div style={{ padding: "16px", display: "flex", flexDirection: "column", gap: "16px", height: "100vh", boxSizing: "border-box" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", borderTop: "1px solid #4A0404", paddingTop: "16px" }}>
        {images.map(img => (
          <ImageItem key={img.id} img={img} onClick={onImageClick} />
        ))}
      </div>
