...
// ⚡ Bolt: Extracted to a memoized component to prevent list-wide re-renders
// Performance impact: Converts O(N) list re-rendering to O(1) when a single image status updates
const ImageRow = memo(({ img, onImageClick }) => {
  return (
    <div
  ...
  return (
    <div style={{ padding: "16px", display: "flex", flexDirection: "column", gap: "16px", height: "100vh", boxSizing: "border-box" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", borderTop: "1px solid #4A0404", paddingTop: "16px" }}>
        {images.map(img => (
          <ImageRow key={img.id} img={img} onImageClick={onImageClick} />
        ))}
      </div>
